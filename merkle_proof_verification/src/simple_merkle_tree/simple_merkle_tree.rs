...
impl MerkleTree {
  ...
  fn next_level_hashes(current_level: Vec<HashOut<GoldilocksField>>) -> Vec<HashOut<GoldilocksField>> {
    // BİR ALT SATIRDA collect_vector yerine collect kullandık.
    let temp: Vec<&[HashOut<GoldilocksField>]> = current_level.chunks(2).into_iter().collect();
  ...
  pub fn build(leaves: Vec<GoldilocksField>, height: usize) -> Self {
    // This panics if length is not a power of 2
    let count_levels = height;
    ...
    for i in 0..(levels.len()) {
        println!("The level {} is {:?}", i, levels[i]);
    }
  ...
  pub fn get_merkle_proof(self, leaf_index: usize) -> Vec<HashOut<GoldilocksField>> {
    ...
    for i in 0..(self.count_levels) {
      ...
      let selected_hash = if updated_index % 2 == 1 {
...
mod tests {
  ...
  #[test]
  fn test_build_merkle_tree_4_leaves() {
    ...
    let _tree: MerkleTree = MerkleTree::build(leaves, 2);
  ...
  #[test]
  fn test_build_merkle_tree_16_leaves() {
    const D: usize = 2;
    ...
    let _tree: MerkleTree  = MerkleTree::build(leaves, 4);
  ...
  #[test]
  fn test_merkle_proof_small_tree() {
    const D: usize = 2;
    type C = PoseidonGoldilocksConfig;
    type F = <C as GenericConfig<D>>::F;
    ...
    let tree: MerkleTree  = MerkleTree::build(leaves, 2);
  ...
  #[test]
  fn test_verify_merkle_proof_16() {
    const D: usize = 2;
    type C = PoseidonGoldilocksConfig;
    type F = <C as GenericConfig<D>>::F;
